            ...
            func open<T: PrimaryKeyedTable>(_: T.Type) {
              withErrorReporting(.sqliteDataCloudKitFailure) {
                ...
                if count > 0 {
                  logger.debug(
                    """
                    deleteLocalData: clearing table
                      Deleting \(count) record(s) from: \(T.databaseTableName)
                    """
                  )
                }
      ...
      case .signOut, .switchAccounts:
        logger.warning(
          ...
          [\(syncEngine.database.databaseScope.label)] handleAccountChange
            🔄 Account change detected: \(changeType == .signOut ? "Sign Out" : "Switch Accounts")
    ...
    package func handleFetchedDatabaseChanges(
      ...
      func deleteRecords(in zoneID: CKRecordZone.ID, db: Database) throws {
        ...
        if !recordTypes.isEmpty {
          logger.warning(
            ...
            [\(syncEngine.database.databaseScope.label)] handleFetchedDatabaseChanges: zone deletion
              ...
              Affected tables: \(recordTypes.joined(separator: ", "))
        ...
        for recordType in recordTypes {
          ...
          func open<T: PrimaryKeyedTable>(_: T.Type) {
            withErrorReporting(.sqliteDataCloudKitFailure) {
              let deletedCount = try T.fetchCount(db)
              if deletedCount > 0 {
                logger.debug(
                  """
                  [\(syncEngine.database.databaseScope.label)] zone deletion progress
                    Deleting \(deletedCount) record(s) from table: \(recordType)
                  """
                )
              }
              try T.delete().execute(db)
        ...
        func open<T: PrimaryKeyedTable>(_: T.Type) async throws {
          ...
          try await userDatabase.write { db in
            do {
              ...
              logger.debug(
                """
                [\(T.databaseTableName)] upsertFromServerRecord foreignKeyConstraint
